...
function App() {
  ...
  return h('div', { className: 'app' },
    ...
    tab === 'account' && h('section', { className: 'panel' },
      ...
      h('div', { className: 'emoji-picker' },
        h('button', { className: 'picker-toggle', onClick: () => setFavoritePickerOpen(v => !v) },
          h('span', null, `Текущий: ${data.favoriteEmoji || '—'}`),
          h('span', null, favoritePickerOpen ? '▴' : '▾')
        ),
        favoritePickerOpen && h('div', { className: 'emoji-rail' }, ...selectableEmojis.map(name => h('button', {
      ...
      h('h3', null, 'Очередь'),
      h('div', { className: 'row' },
        h('select', { value: queueEmoji, onChange: e => setQueueEmoji(e.target.value) }, ...selectableEmojis.map(e => h('option', { key: e }, e))),
      ...
      h('h3', null, 'Батл'),
      h('div', { className: 'row' },
        h('select', { value: battleEmoji, onChange: e => setBattleEmoji(e.target.value) }, ...selectableEmojis.map(e => h('option', { key: e }, e))),
