...
function App() {
  ...
  const raceUnits = (data.race?.units || []).map(u => h('div', { className: 'unit', key: u.playerNumber },
    ...
    h('div', { className: 'actions' },
      h('button', { onClick: () => act('vote', { matchId: data.race.matchId, playerNumber: u.playerNumber, amount: voteAmount }) }, 'Голос'),
  ...
  return h('div', { className: 'app' },
    ...
    tab === 'race' && h('section', { className: 'panel' },
      ...
      h('p', { className: 'subtitle' }, 'Голос доступен при достаточном балансе. Сумму можно менять.'),
      h('div', { className: 'row' }, h('input', { type: 'number', min: 1, value: voteAmount, onChange: e => setVoteAmount(Number(e.target.value || 1)) })),
      ...raceUnits
    ),
    tab === 'account' && h('section', { className: 'panel' },
      ...
      h('h3', null, 'Баланс'),
      h('div', { className: 'row' },
        h('input', { type: 'number', min: 1, value: topupAmount, onChange: e => setTopupAmount(Number(e.target.value || 1)) }),
        h('button', { onClick: () => act('topup', { amount: topupAmount }) }, 'Пополнить через ⭐')
      ),
      h('h3', null, 'Вывод'),
      h('div', { className: 'row' },
        h('input', { type: 'number', min: 100, value: withdrawAmount, onChange: e => setWithdrawAmount(Number(e.target.value || 100)) }),
        h('button', { onClick: () => act('withdraw', { amount: withdrawAmount }) }, 'Создать запрос на вывод')
      ),
